...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    exposure_file = os.path.join(parent_dir,"exposure.json")
    meta_data = None
    exposure_data = None
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        exposure_time, ISO, f_number = None, None, None
        if meta_data is not None:
            exposure_time = meta_data[extr.name]["ExposureTime"]
            ISO = meta_data[extr.name]["ISOSpeedRatings"]
            f_number = meta_data[extr.name]["FNumber"]
        if exposure_data is not None:
            exposure_time = exposure_data[extr.name]

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height,
                              exposure_time=exposure_time, ISO=ISO, f_number=f_number)
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    tsv = glob.glob(os.path.join(path, '*.tsv'))
    train_test_split = None if len(tsv) == 0 else pd.read_csv(tsv[0], sep='\t')

    if eval:
        if train_test_split is not None:
            train_test_split_sorted = train_test_split.sort_values('filename', ascending=True)
